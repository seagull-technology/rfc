@php
    ...
    $fixedDocumentType = $fixedDocumentType ?? null;
    $documentType = (string) ($fixedDocumentType ?: ($attachment['document_type'] ?? ''));
@endphp

<div class="ministry-personal-details-form__attachment" data-ministry-attachment-row data-document-type="{{ $documentType }}" data-stored="{{ $storedAttachment ? 'true' : 'false' }}">
    ...
    @if ($fixedDocumentType)
        <input type="hidden" name="{{ $attachmentInputPrefix }}[document_type]" value="{{ $fixedDocumentType }}">
    @endif

    <div class="d-flex align-items-center justify-content-between gap-3 mb-3">
        <strong>
            {{ $fixedDocumentType
                ? __('app.applications.ministry_interior_personal_details.options.attachment_type.'.$fixedDocumentType)
                : __('app.applications.ministry_interior_personal_details.attachment_record').' '.(is_numeric($attachmentIndex) ? ((int) $attachmentIndex + 1) : 1) }}
        </strong>
        @unless ($readOnly || $fixedDocumentType)
            ...
            <div class="col-12 col-lg-7">
                ...
                <select class="form-select" id="{{ $attachmentIdPrefix }}_document_type" name="{{ $attachmentInputPrefix }}[document_type]" @disabled($readOnly)>
                    ...
                    @foreach (['professional_license', 'kinship_proof', 'passport_copy', 'sponsor_residence', 'foreign_residence'] as $option)
                        <option value="{{ $option }}" @selected($documentType === $option)>{{ __('app.applications.ministry_interior_personal_details.options.attachment_type.'.$option) }}</option>
                    @endforeach
                </select>
            </div>
        @endunless
        <div class="col-12 {{ $fixedDocumentType ? '' : 'col-lg-5' }}">
            @if ($readOnly)
